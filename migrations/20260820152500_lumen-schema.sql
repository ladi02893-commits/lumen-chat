...
create table if not exists public.conversations (
  ...
  auto_delete_seconds integer check(auto_delete_seconds between 60 and 604800),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
begin
  ...
  expiration := case mode
    ...
    when 'custom' then now() + make_interval(secs => seconds)
    else null
  end;
...
-- InsForge Auth trigger to auto-create profile & settings on signup
...
declare
  ...
  new_profile_id uuid;
begin
  -- Extract name safely from InsForge profile/metadata jsonb columns
  raw_name := coalesce(
    case when new.profile is not null then new.profile->>'name' else null end,
    case when new.profile is not null then new.profile->>'full_name' else null end,
    case when new.metadata is not null then new.metadata->>'name' else null end,
    case when new.metadata is not null then new.metadata->>'full_name' else null end,
    split_part(coalesce(new.email, 'User'), '@', 1)
  );

  -- Extract username safely
  raw_user := coalesce(
    case when new.profile is not null then new.profile->>'username' else null end,
    case when new.metadata is not null then new.metadata->>'username' else null end,
    split_part(coalesce(new.email, 'user'), '@', 1)
  );

  -- Clean username to match regex ^[a-z0-9_]{3,24}$
  base_username := lower(regexp_replace(coalesce(raw_user, 'user'), '[^a-z0-9_]', '_', 'g'));
  ...
  -- Ensure uniqueness
  while exists (select 1 from public.profiles where username = final_username and auth_user_id <> new.id) loop
    counter := counter + 1;
    final_username := substr(base_username, 1, 18) || '_' || counter;
  end loop;
  ...
  insert into public.profiles (
    ...
    new.id,
    substr(coalesce(raw_name, 'User'), 1, 80),
    final_username,
    coalesce(new.email, ''),
    case when new.profile is not null then new.profile->>'avatar_url' else null end
